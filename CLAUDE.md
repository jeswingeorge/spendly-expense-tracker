...
- **Step 3 (login/logout)** — `POST /login` verifies credentials with
  `check_password_hash` and starts a session (`session["user_id"]`); `/logout`
  clears it. A `@app.before_request` loader sets `g.user`, and `base.html` swaps
  the nav between signed-out and signed-in state. `get_user_by_id()` helper added
  to `database/db.py`. See `.claude/specs/03-login-and-logout.md`.
...
- **`app.py`** — the entire application: one Flask app, all routes defined flat in this
  file. Routes split into "real" (`/`, `/register`, `/login`, `/logout`, `/terms`,
  `/privacy`) and placeholder stubs (`/profile`, `/expenses/add`,
  `/expenses/<id>/edit`, `/expenses/<id>/delete`) that currently return plain strings
  labeled "coming in Step N". A `@app.before_request` hook loads the current user
  into `g.user` from `session["user_id"]`. There is no blueprint / app-factory structure.
...
- **`templates/`** — Jinja2. Every page `{% extends "base.html" %}` and fills
  ...
  internal links and static assets. `POST /register` (Step 2) and `POST /login`
  (Step 3) are implemented; `base.html` shows a signed-in nav when `g.user` is set.
...
| `GET, POST /login` | Implemented — Step 3 (login/logout) |
| `GET /logout` | Implemented — Step 3 (login/logout) |
